...
from modules.decoding.decoder_packets import DecoderPackets

from modules.processing.packets_processor import PacketProcessor
from modules.responding.responses_maker import ResponseMaker
    ...
    def __packet_processor(self):
        while True:
            try:
                ...
                parsed_packets = DecoderPackets(raw_packets).parse().get_result()
                print("Parsed:\n", parsed_packets)

                # construct responses and requests to be sent back before processing the packets
                responses = []
                responses.extend(ResponseMaker(parsed_packets).make())

...
from modules.tcp_server.models.protocol_400B import Protocol400BModel

from ..section.stat_data import StatDataDecoder
from modules.tcp_server.enums.dtc_data.fault_flag import DTCFaultFlagEnum
from modules.tcp_server.enums.dtc_data.dtc_attr import DTCAttrEnum
...
class Protocol400BDecoder(Decoder):
    ...
    def protocol_data(self):
        self.model.fault_flag = DTCFaultFlagEnum().get(self.set_part(self.data[self.position: self.move(1)]).to_hex().hex_int().get_part())
        self.model.fault_count = self.set_part(self.data[self.position: self.move(1)]).to_hex().hex_int().get_part()

        for _ in range(self.model.fault_count):
            model = self.model.getDTC_Model()
            model.code = self.data[self.position: self.move(2)]

            attr = self.set_part(self.data[self.position: self.move(1)]).to_hex().to_bin().get_part()

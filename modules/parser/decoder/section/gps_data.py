from modules.parser.decoder.decoder import Decoder
from modules.parser.models.gps_model import GPS_Model
...
class GPSDataDecoder(Decoder):
    ...
    def gps_count(self):
        self.model.gps_count = self.set_part(self.data[0:self.move(1)]).to_hex().hex_int().get_part()
        return self
    ...
    def gps_data(self):
        for i in range(self.model.gps_count):
            ...
            model.latitude = self.set_part(data[6:10]).to_hex().reverse_bytes().hex_int().get_part() / 3600000
            model.longitude = self.set_part(data[10:14]).to_hex().reverse_bytes().hex_int().get_part() / 3600000
            model.speed = self.set_part(data[14:16]).to_hex().reverse_bytes().hex_int().get_part() * 0.022369
            model.direction = self.set_part(data[16:18]).to_hex().reverse_bytes().hex_int().get_part()
